...
async function main () {
  if (await exists(targetPath)) {
    console.log(`Found node-raylib@${process.env.npm_package_version} for ${process.platform}/${process.arch}`)
    process.exit(0)
  }
  ...
  console.log(`Installing node-raylib@${process.env.npm_package_version} for ${process.platform}/${process.arch}`)
